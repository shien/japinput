...
**動作確認:**
- `cargo test` でパーサーのユニットテストがパスすること
- 正常行・コメント行・空行・アノテーション付き行それぞれのテストケースが網羅されていること
...
**動作確認:**
- `cargo test` で `Dictionary` のユニットテストがパスすること
- `tests/fixtures/test_dict.txt` を使った結合テストで `load_from_file` → `lookup` の一連の流れを確認
...
**動作確認:**
- `tests/fixtures/test_dict.txt` が正しい SKK 辞書形式であること
- `.gitignore` に `dict/` が含まれ、大容量辞書がリポジトリに混入しないこと (`git status` で確認)
...
**動作確認:**
- `cargo run` でひらがなを入力し、辞書から漢字候補が返ることを手動確認
